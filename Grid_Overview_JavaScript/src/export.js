...
Fonts.Ipaexg = new wjcPdf.PdfFont('ipaexg');
//
...
export class ExportService {
    startExcelExport(flex, ctx) {
        ...
        wjcGridXlsx.FlexGridXlsxConverter.saveAsync(flex, {
            ...
            console.log('Excelエクスポートが完了しました');
            this._resetExcelContext(ctx);
        }, err => {
            console.error(`Excelエクスポートが失敗しました: ${err}`);
            this._resetExcelContext(ctx);
        }, prg => {
            if (ctx.preparing) {
                ctx.exporting = true;
        ...
        console.log('Excelエクスポートが開始されました');
    }
    cancelExcelExport(ctx) {
        wjcGridXlsx.FlexGridXlsxConverter.cancelAsync(() => {
            console.log('Excelエクスポートがキャンセルされました');
            this._resetExcelContext(ctx);
        });
    }
    exportToPdf(flex, options) {
        wjcGridPdf.FlexGridPdfConverter.export(flex, PdfExportDocName, {
            embeddedFonts: [{ source: 'https://demo.grapecity.com/wijmo/sample/fonts/ipaexg.ttf', name: 'ipaexg' }],
            maxPages: 100,
    ...
    _formatPdfCountryCell(e, countryMap) {
        ...
        if (this._isCountryExist(countryName, countryMap)) {
            ...
            // draw country name
            e.canvas.drawText(countryName, contentRect.left + image.width + 3, e.textTop, { font: Fonts.Ipaexg });
        }
    ...
    _formatPdfColorCell(e, colorMap) {
        ...
        if (this._isColorExist(colorName, colorMap)) {
            ...
            e.canvas.paths
                ...
                .fillAndStroke(wjcCore.Color.fromString(['Black', 'White', 'Red', 'Green', 'Blue'][colorMap.getKeyValue(colorName)]), wjcCore.Color.fromString('gray'));
            // draw color name
            e.canvas.drawText(colorName, contentRect.left + imageWidth + 3, e.textTop, { font: Fonts.Ipaexg });
        }
        // cancel standard cell content drawing
        e.cancel = true;
    }
